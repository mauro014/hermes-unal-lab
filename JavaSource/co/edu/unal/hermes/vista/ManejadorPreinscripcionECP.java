...
public class ManejadorPreinscripcionECP extends ManejadorBase {
	...
	protected List<SelectItem> tipoSectorEntidadPreinscritoItemList;
	...
	private Ciudad lugar_expedicion_atributo;
	...
	private boolean mostrarDirePagoVirtual;
	...
	public ManejadorPreinscripcionECP() {

		cargarListasParticipantes();

		// opcionesTerminos = "No";
		mostrarBotonGuardar = false;
		mostrarDirePagoVirtual = false;
															...
															// curso...
		{
			...
			FacesContext.getCurrentInstance().addMessage(
					...
					new FacesMessage(FacesMessage.SEVERITY_ERROR,
							"El curso no tiene una oferta vigente", ""));
	...
	public boolean esEmpleado() {
		...
		if (!documento.equals("")) {
			cons_empleado = "Select pp from Investigador pp where pp.id.documento like '"
					+ this.documento
					+ "'"
					+ "and pp.id.tipoDocumento like " 
					+ "'"
	...
	public boolean esEgresado() {
		...
		if (!documento.equals("")) {
			cons_estudiante = "Select pp from Estudiante pp where pp.id.documento like '"
					+ this.documento
					+ "' and pp.id.tipoDocumento like '"
					+ this.tipo_doc + "'" + "and pp.interno in ('N')"; // Estudiantes
																		// activos
			...
			if (lista_estudiante.size() != 0) {
				estudiante = (Estudiante) lista_estudiante.get(0);
				System.out.println("   -> ES EGRESADO  <-  "
						+ estudiante.getId().getDocumento());
				return true;
			} else
				return false;
	...
	public void datosPersona() {
		if (persona != null) {
			...
			try{
				if(persona.getCiudadExpedicion() != null){
					setLugar_expedicion(persona.getCiudadExpedicion().getId());
				}
			}catch(Exception e){
	...
	public void construir_objetos() {
		String cons_pais, cons_lugar_expedicion, cons_depto, cons_ciud, cons_formacion, cons_tddoc, cons_otros_intereses, cons_tpvinculacion, cons_oferta, cons_ocupacion, cons_medio;
		...
		cons_tddoc = "select pp from TipoDocumento pp where  pp.id = '"
				+ this.tipo_doc + "'";
		List lista_tpdoc = servicioGeneral.obtenerObjetos(cons_tddoc);
		tipo_documento = (TipoDocumento) lista_tpdoc.get(0);
		...
		if (pais.equals("CO")) {
			cons_depto = "select dd from Departamento dd where  dd.id  = '"
					+ this.departamento + "'";
			List lista_depto = servicioGeneral.obtenerObjetos(cons_depto);
			departamento_atributo = (Departamento) lista_depto.get(0);

			cons_ciud = "select cc from Ciudad cc where  cc.id = '"
					+ this.ciudad + "'";
			List lista_ciud = servicioGeneral.obtenerObjetos(cons_ciud);
			ciudad_atributo = (Ciudad) lista_ciud.get(0);
		}
		
		cons_lugar_expedicion = "select cc from Ciudad cc where  cc.id = '"
				+ this.lugar_expedicion + "'";
		
		List lista_lugar_expedicion = servicioGeneral.obtenerObjetos(cons_lugar_expedicion);
		
		lugar_expedicion_atributo = (Ciudad) lista_lugar_expedicion.get(0);
		
		if (this.formacion_academica != null && !this.formacion_academica.equals("")) {
			cons_formacion = "select pp from TipoFormacion pp where  pp.id = '"
					+ this.formacion_academica + "'";
			List lista_tpformacion = servicioGeneral.obtenerObjetos(cons_formacion);
			formacion_academica_atributo = (TipoFormacion) lista_tpformacion.get(0);
		}else{
		...
		if (!otros_intereses.equals("")) {
			if (!otros_intereses.equals("N")) {
				cons_otros_intereses = "select pp from AgendaConocimiento pp where  pp.id = "
						+ this.otros_intereses;
				List lista_otrosIntereses = servicioGeneral
						.obtenerObjetos(cons_otros_intereses);
				otros_intereses_atributo = (AgendaConocimiento) lista_otrosIntereses
		...
		if(medio_publicidad != null){
			if (!medio_publicidad.equals("")) {
				cons_medio = "select pp from DominioDetalle pp where  pp.identificador.tipo = '"
						+ this.medio_publicidad + "'";
				List lista_medio = servicioGeneral.obtenerObjetos(cons_medio);
				medio_publicidad_atributo = (DominioDetalle) lista_medio.get(0);
			}
		...
		try{
			cons_tpvinculacion = "select pp from DominioDetalle pp where  pp.identificador.tipo = '"
					+ this.tipo_vinculacion + "'";
			List lista_tpVinculacion = servicioGeneral
					.obtenerObjetos(cons_tpvinculacion);
			tipo_vinculacion_atributo = (DominioDetalle) lista_tpVinculacion.get(0);
	...
	private boolean guardarPersonaInvestigadorBasico() {
		...
		try {
			...
			if (lista_persona.size() > 0) {
				...
				investigador.setCiudadExpedicion(lugar_expedicion_atributo);
				...
				if (!tiRol) {
					Rol rolExt = new Rol();
					rolExt.setId("FO");
					PersonaRol perRol = new PersonaRol(investigador.getId().getTipoDocumento(), investigador.getId().getDocumento(), "FO");
										
					servicioPersona.crearRoles(perRol);
				}
		
				servicioPersona.actualizarPersonaDatosBasicos(investigador);
				...
				persona.setCiudadExpedicion(lugar_expedicion_atributo);
	...
	public void enviarCorreoInvestigacion(){
		try {
			...
			if (persona != null) {
				 ...
				 mensajeConfirmacion = "\nInformamos que la preinscripción al " + curso.getNombre() + " se ha realizado con éxito. "
				 + "\n\n";
				 ...
				 + "";
					...
					correo.adicionarDireccion(preins_ecp_objeto.getInvestigador()
							.getEmail());
					//correo.adicionarCopiaOculta("sisii_nal@unal.edu.co");
					correo.setAsunto("PREINSCRIPCIÓN " + curso.getNombre().toUpperCase());
					correo.setCuerpo(mensajeConfirmacion);
	...
	public void enviarCorreoECP() {
		try {
			...
			if (persona != null) {
				...
				List lista = servicioGeneral.obtenerListaObjetos("CorreoPlantilla co where co.id = 161");
				...
				correoActual.setCuerpo(correoActual.getCuerpo().replaceAll("<<NOMBRE_ASISTENTE>>", preins_ecp_objeto.getInvestigador().getNombre1() + " " + preins_ecp_objeto.getInvestigador().getApellido1()));
				
				correoActual.setCuerpo(correoActual.getCuerpo().replaceAll("<<NOMBRE>>", preins_ecp_objeto.getInvestigador().getNombre1() + " " +
																						 preins_ecp_objeto.getInvestigador().getNombre2() + " " +
																						 preins_ecp_objeto.getInvestigador().getApellido1() + " " +
																						 preins_ecp_objeto.getInvestigador().getApellido2() 
																						 ));
				correoActual.setCuerpo(correoActual.getCuerpo().replaceAll("<<DOCUMENTO>>", 	 preins_ecp_objeto.getInvestigador().getId().getDocumento()));
				correoActual.setCuerpo(correoActual.getCuerpo().replaceAll("<<TIPO_DOCUMENTO>>", preins_ecp_objeto.getInvestigador().getId().getTipoDocumento()));
				correoActual.setCuerpo(correoActual.getCuerpo().replaceAll("<<CORREO>>", 		 preins_ecp_objeto.getInvestigador().getEmail()));
				correoActual.setCuerpo(correoActual.getCuerpo().replaceAll("<<TELEFONO_FIJO>>",  preins_ecp_objeto.getInvestigador().getTelefono()));
				correoActual.setCuerpo(correoActual.getCuerpo().replaceAll("<<TELEFONO_MOVIL>>", preins_ecp_objeto.getTel_movil()));
				correoActual.setCuerpo(correoActual.getCuerpo().replaceAll("<<DIRECCION>>", 	 preins_ecp_objeto.getInvestigador().getDireccion()));
				correoActual.setCuerpo(correoActual.getCuerpo().replaceAll("<<CIUDAD>>", 		 preins_ecp_objeto.getCiudad().getNombre()));
				
				String enlace = "http://www.hermes.unal.edu.co/pages/Extension/CursoECP.xhtml?idCurso="+ curso.getId() +"&opcion=1";
				System.out.println("Enlace:" + enlace);
				correoActual.setCuerpo(correoActual.getCuerpo().replaceAll("<<ENLACE>>", enlace));
				
				Investigador inv = servicioPersona.obtenerInvestigador(preins_ecp_objeto.getInvestigador().getId());
				InvestigadorExterno invExt = servicioPersona.obtenerInvestigadorExterno(inv.getId());
				
				if(preins_ecp_objeto.getEsExterno().equals("S") && invExt != null)
				{	
					
					correoActual.setCuerpo(correoActual.getCuerpo().replaceAll("<<USUARIO_EXTERNOS>>", servicioPersona.login(invExt) + " (Siempre en mayúsculas)"));
					correoActual.setCuerpo(correoActual.getCuerpo().replaceAll("<<CLAVE_EXTERNOS>>", invExt.getContrasena().toString()));
				}	
				else 
				{
					correoActual.setCuerpo(correoActual.getCuerpo().replaceAll("<<USUARIO_EXTERNOS>>", ""));
					correoActual.setCuerpo(correoActual.getCuerpo().replaceAll("<<CLAVE_EXTERNOS>>", ""));
				}	
				...
				//correo.adicionarCopiaOculta("sisii_nal@unal.edu.co");
				correo.adicionarCopiaOculta("hermesecp_nal@unal.edu.co");
	...
	public void guardarPreinscripcion() {
		...
		if (lista_preins.size() != 0) // LA PREINSCRIPCION YA EXISTE // [ACTUALIZAR]
		{
			ins_act = "A";
		...
		if (guardarPersonaInvestigadorBasico()) {
			...
			if (documento != "" && primer_nombre != "" && primer_apellido != ""
				...
				if (id_oferta != -1 && curso != null) {
					...
					//enviar datos a ECP
					ConexionBDECP.StoreProcedure("DECLARE res NUMBER; BEGIN res := UEC_SCHEMA.UECF_INSERTA_PREINSCRITO('"+documento+"','"+tipo_doc+"','"+id_oferta+"'); END;");
					mostrarDirePagoVirtual = true;
	...
	public String salirPreinscripcion(){
		sesion.removeAttribute("ManejadorPreinscripcionECP");	
		return "salir";
	}

	public void guardarPreinscripcionInvestigacion() {
	...
	private void cargarFormacionAcademica() {
		...
		String consulta = "select ff from TipoFormacion ff where ff.id in "
				+ "('ME','UT','UL','UM','BS','TL','11','NA','UN','PD','PG','NF','ES','EM','MA','MM','PH','PR','DO','TC', 'PE', 'EB', 'PI')";
	...
	public void cargarSectorPreinscrito() {
		tipoSectorEntidadPreinscritoItemList = new ArrayList<SelectItem>();
		String consulta = "select ff from DominioDetalle ff, Dominio d where ff.identificador.id = d.id and d.tipo like 'SECTOR_PREINS_SEG_CONGRESO_INNO_SOCIAL' ";
		List lista = servicioGeneral.obtenerObjetos(consulta);

		for (int i = 0; i < lista.size(); i++) {
			DominioDetalle tipo_vinculacion = (DominioDetalle) lista.get(i);
			tipoSectorEntidadPreinscritoItemList.add(new SelectItem(tipo_vinculacion
					.getIdentificador().getTipo(), tipo_vinculacion
					.getDescripcion()));
		}
	}
	...
	public static void setLogger(Logger logger) {
		ManejadorPreinscripcionECP.logger = logger;
	}
	...
	public void cargarListasParticipantes() {
		...
		manizales.put("24334177",
				"Manizales | Viernes, 06 de septiembre de 2013");
		manizales.put("10143816",
				"Manizales | Viernes, 06 de septiembre de 2013");
		...
		villavicencio.put("11376059",
				"Villavicencio | Viernes, 27 de septiembre de 2013");
		villavicencio.put("80047706",
				"Villavicencio | Viernes, 27 de septiembre de 2013");
		villavicencio.put("17390690",
				"Villavicencio | Viernes, 27 de septiembre de 2013");
		villavicencio.put("17389661",
				"Villavicencio | Viernes, 27 de septiembre de 2013");
		villavicencio.put("80175766",
				"Villavicencio | Viernes, 27 de septiembre de 2013");
		villavicencio.put("86080031",
				"Villavicencio | Viernes, 27 de septiembre de 2013");

		bucaramanga.put("91286021",
				"Bucaramanga | Viernes, 20 de septiembre de 2013");
		bucaramanga.put("91218674",
				"Bucaramanga | Viernes, 20 de septiembre de 2013");
		bucaramanga.put("1098622174",
				"Bucaramanga | Viernes, 20 de septiembre de 2013");

		bogotaUno.put("9870016",
				"Bogotá (1) | Viernes, 06 de septiembre de 2013");
		bogotaUno.put("79410911",
				"Bogotá (1) | Viernes, 06 de septiembre de 2013");
		bogotaUno.put("11809178",
				"Bogotá (1) | Viernes, 06 de septiembre de 2013");
		bogotaUno.put("19404247",
				"Bogotá (1) | Viernes, 06 de septiembre de 2013");
		bogotaUno.put("79684468",
				"Bogotá (1) | Viernes, 06 de septiembre de 2013");
		bogotaUno.put("1053802877",
				"Bogotá (1) | Viernes, 06 de septiembre de 2013");

		bogotaDos.put("52337478",
				"Bogotá (2) | Viernes, 13 de septiembre de 2013");
		bogotaDos.put("41753294",
				"Bogotá (2) | Viernes, 13 de septiembre de 2013");
		bogotaDos.put("14224506",
				"Bogotá (2) | Viernes, 13 de septiembre de 2013");
		bogotaDos.put("1053786454",
				"Bogotá (2) | Viernes, 13 de septiembre de 2013");
		bogotaDos.put("1072639383",
				"Bogotá (2) | Viernes, 13 de septiembre de 2013");
		bogotaDos.put("52428273",
				"Bogotá (2) | Viernes, 13 de septiembre de 2013");

		sanAndres.put("52422768",
				"San Andrés | Viernes, 06 de septiembre de 2013");
		sanAndres.put("72288888",
				"San Andrés | Viernes, 06 de septiembre de 2013");
		sanAndres.put("39154891",
				"San Andrés | Viernes, 06 de septiembre de 2013");
		sanAndres.put("18012005",
				"San Andrés | Viernes, 06 de septiembre de 2013");

	}

	public boolean isMostrarDirePagoVirtual() {
		return mostrarDirePagoVirtual;
	}

	public void setMostrarDirePagoVirtual(boolean mostrarDirePagoVirtual) {
		this.mostrarDirePagoVirtual = mostrarDirePagoVirtual;
	}
	...
	public Ciudad getLugar_expedicion_atributo()
	{
		return lugar_expedicion_atributo;
	}

	public void setLugar_expedicion_atributo(Ciudad lugar_expedicion_atributo)
	{
		this.lugar_expedicion_atributo = lugar_expedicion_atributo;
	}

	public List<SelectItem> getTipoSectorEntidadPreinscritoItemList() {
		return tipoSectorEntidadPreinscritoItemList;
	}

	public void setTipoSectorEntidadPreinscritoItemList(
			List<SelectItem> tipoSectorEntidadPreinscritoItemList) {
		this.tipoSectorEntidadPreinscritoItemList = tipoSectorEntidadPreinscritoItemList;
	}	
