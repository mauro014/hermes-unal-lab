...
public class ManejadorSemillerosConsultaUAB extends ManejadorSemillerosConsultaBase {

	private static final long serialVersionUID = 1L;

	public ManejadorSemillerosConsultaUAB() {
		super();
	...
	private void init() {
		semilleros = servicioGeneral.obtenerObjetos(Semillero.class, "select s from Semillero s, SemilleroIntegrante si, InvestigadorInterno ii where si.semillero.id = s.id and si.tipo.id = 'DD' "
				+ "and ii.id.documento = si.integrante.id.documento and ii.dependencia.id in ('"+investigadorInterno.getDependencia2().getId()+"')" );
		setListaSolicitudes(new ArrayList<SemilleroSolicitud>());
		for (Semillero s : semilleros) {
			if (s.getLider()!=null && s.getLider().getDependencia()!=null && Sede.SEDES_ANDINAS.contains(s.getLider().getDependencia().getSede().getId().toString())
				...
				for (SemilleroSolicitud ssol : s.getSolicitudes()) {
					if (ssol.getFechaRespuestaUAB() != null) {
