...
public class ManejadorIndicadoresSAC extends ManejadorBase {
	...
	public ManejadorIndicadoresSAC() {
	...
	private void cargarIconos() {
		...
		if (esDecano) {
			iconosAgregados.add(new Icono("Educación contínua y permanente", "indicaUEC"));
			iconosAgregados.add(new Icono("Otras modalidades de extensión", "indicaSAC"));
		}
		if (esVicerector) {
			iconosAgregados.add(new Icono("Educación contínua y permanente", "indicaUEC"));
			iconosAgregados.add(new Icono("Otras modalidades de extensión", "indicaSAC"));
	...
	public void accion(){
		...
		String sql = "SELECT '"+ConexionBDECP.APEX_PROD+"f?p=198:701:::::P701_USUARIO_HERMES,F198_HERMES_BRANCH:'||uec_schema.uecf_hash_hermes(upper(trim('" + personaActual.getUid().toUpperCase() + "')),'USUARIOECP')||',263' as ENLACE from dual";
		
		 	url = ConexionBDECP.execQueryLink(sql);
		}
		else if(accion == "indicaSAC"){
			System.out.println("login SAC user: " + personaActual.getUid().toUpperCase());
			String sql = "SELECT '"+ConexionBDECP.APEX_PROD+"f?p=111:701:::::P701_USUARIO_HERMES,F209_HERMES_BRANCH:'||sac_schema.sieif_hash_hermes(upper(trim('" + personaActual.getUid().toUpperCase() + "')),'USUARIOSAC')||',71' as ENLACE from dual";
