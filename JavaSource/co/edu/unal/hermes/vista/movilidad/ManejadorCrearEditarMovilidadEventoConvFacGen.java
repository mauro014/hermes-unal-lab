...
public class ManejadorCrearEditarMovilidadEventoConvFacGen extends ManejadorBaseMovilidad {

    private static final long serialVersionUID = -871835047986122703L;
    ...
    private List<ArchivoMovilidadDE> listaArchivosObligatoriosSel;
    ...
    private SelectItem[] caracterEventoItem = { new SelectItem("1", "Internacional en Colombia"),
            new SelectItem("2", "Internacional en el extranjero"), new SelectItem("3", "Nacional") };
    ...
    public ManejadorCrearEditarMovilidadEventoConvFacGen() {
        ...
        if(idMovilidadEd != null){
        	...
        	listaArchivosObligatoriosSel.addAll(mde.getArchivos());
        	listaPonencias.addAll(mde.getListaPonencias());
        	try
			{
				buscarPersona();
			}
        	...
        	nombreEvento = mde.getEvento();
        	tituloPonencia = mde.getTitulo();
        	ciudadEvento = mde.getCiudad();
        	costoTiquetes = mde.getCostotiquete();
        	resumenPonencia = mde.getResumen();
        	otroTipoPonencia = mde.getOtroTipoPonencia();
        	costoInscripcion = mde.getCostoevento();
        	valorViaticos = mde.getValorViaticos();
        	aporteFacultad = mde.getAportefacultad();
        	fechaInicioEvento = mde.getFechainicial();
        	fechaFinEvento = mde.getFechafinal();
        	aceptacionFacultad = mde.getAceptacion();
        	universidad = mde.getUniversidad();
        	tituloAspira= mde.getTituloAspira();
        	infoConvocatoriasExternas = mde.getInfoConvocatoriaExterna();
        	detalleConvocatoriaExterna = mde.getDescConvocatoriaExterna();
        	caracterEvento = mde.getCaracterEvento();
        	medioTransporte = mde.getMedioTransporte();
        	nombreUniversidadPonencia= mde.getNombreUniversidad();
        	submodalidad = mde.getSubModalidadConvocatoria();
        	valorTotalMovSolicitado = mde.getValorTotalMovilidadSolicitado();
        	
        	idTipoPonencia = mde.getPonencia().getId().toString();
        	
        	if(mde.getDescConvocatoriaExterna() != null){
        		siConvExt = true;
        	}
                    ...
                    if (convActual.getEsParaGrupos()) {
        				...
        				listaOpcionsSeleDos = servicioGeneral
        			   	 ...
        			   	 for (int i = 0; i < listaOpcionsSeleDos.size(); i++) {
        			   		...
        			   		listaSubmodalidades[i] = new SelectItem(dd.getIdentificador()
        ...
        if(tipoPonenciaPar != null){
        	if(!tipoPonenciaPar.equals("")){
        		listaTipoPonencia = servicioGeneral.obtenerListaObjetos("TipoPonencia where id in (" + tipoPonenciaPar + ") order by descripcion");
        	}
        }else{
        	listaTipoPonencia = servicioGeneral.obtenerListaObjetos("TipoPonencia where id in (2,4,9) order by descripcion");
        }
        ...
        if(restriccionArchivosConv != null){
        	if(restriccionArchivosConv.equals("CF_767")){
    			caracterEventoItem =  new SelectItem[2];
    			caracterEventoItem[0] = new SelectItem("1", "Internacional en Colombia");
            	caracterEventoItem[1] = new SelectItem("3", "Nacional");
    		}
        }
    ...
    public void adicionarPonencia() {
        ...
        if (validarTituloYEvento(mde)) {
            ...
            listaTipoMovilidad = servicioGeneral.obtenerListaObjetos("TipoMovilidad where id ='CF_MOV8'");
    ...
    public void buscarPersona() throws SQLException {
        ...
        if (!bandera) {
            sSql = " SELECT COUNT(1) " + " FROM " + " HER_MOVILIDAD_DOCENTES_EVENTOS " + " WHERE " + " MOV_ID_PER ='"
                    + documento + "'" + " AND MOV_APROB = 'SI' " + "AND MOV_FEC_SOL BETWEEN TO_DATE('12/01/" + anoAnt
                    + "', 'MM/DD/YYYY') AND TO_DATE('11/30/" + anoAct + "', 'MM/DD/YYYY')";
        }

        nExiste = servicioGeneral.existeMovilidad(sSql);

        if (nExiste < 0) {
            noExiste = "El investigador ya tiene aprobada una movilidad para este año en esta convocatoria.";
    ...
    public void guardarConvocatoriaFacultad() {
        ...
        mde.setEvento(cortarCadena(nombreEvento, 2000));
        mde.setTitulo(cortarCadena(tituloPonencia, 2000));
        mde.setCiudad(cortarCadena(ciudadEvento, 255));
        mde.setCostotiquete(costoTiquetes);
        mde.setResumen(cortarCadena(resumenPonencia, 3900));
        ...
        mde.setValorOtrosAportes(valorAporteOtrasDep);
        mde.setDependenciaOtrosAportes(depValorAporteOtrasDep);
        ...
        mde.setSubModalidadConvocatoria(submodalidad);
        mde.setValorTotalMovilidadSolicitado(valorTotalMovSolicitado);
        ...
        if (validar(mde)) {
            ...
            if (listaPonencias.size() > 0) {
                for (int i = 0; i < listaPonencias.size(); i++) {
                    MovilidadPonencia mop = (MovilidadPonencia) listaPonencias.get(i);
            ...
            if (listaArchivosObligatoriosSel != null && listaArchivosObligatoriosSel.size() > 0) {
                for (int i = 0; i < listaArchivosObligatoriosSel.size(); i++) {
                    ArchivoMovilidadDE amovAux1 = (ArchivoMovilidadDE) listaArchivosObligatoriosSel.get(i);
...
//            if (opcion1 && opcion2) {
//                noExiste1 = "Solicitud de movilidad guardada correctamente con el código " + mde.getId() + ". Un correo electrónico confirmando su registro se ha enviado a su cuenta de correo electrónico.";
//            } else {
//                noExiste1 = "Solicitud de movilidad guardada correctamente con el código " + mde.getId() + ". Ha ocurrido un error al enviar el correo electrónico con la confirmación de registro de la movilidad. ";
//            }
...
//                noExiste1 = "Solicitud de movilidad guardada correctamente con el código " + mde.getId() + ". Un correo electrónico confirmando su registro se ha enviado a su cuenta de correo electrónico.";
//            } else {
//                noExiste1 = "Solicitud de movilidad guardada correctamente con el código " + mde.getId() + ". Ha ocurrido un error al guardar la movilidad. ";
//            }
            noExiste1 = "La movilidad con código" + mde.getId() + "se ha guardado y enviado correctamente al usuario a través del "
					+ "sistema Hermes; sin embargo, no se ha enviado correo, les recomendamos comunicarse con "
					+ "la Coordinación de Investigación de la facultad o sede a la que pertenece para realizar "
    ...
    private boolean validar(MovilidadDocentesExterior mov) {
        ...
        /*
         * if (mov.getResolucionviaje().equals("")) { String error =
         * "La resolución de comisón es obligatoria \n"; errores[7] = error;
         * bErrorResolucion = true; result = false; }
         */
        ...
        if(tipoConvocatoria.equals("CF_MOV8") && restriccionTiquetes != null){ 
        	if(mov.getCostoevento() == null){
        	...
        	if((mov.getCostoevento() + mov.getCostotiquete() + mov.getValorViaticos()) > restriccionTiquetes){
        		  String error = "El valor total de la movilidad no puede ser superior a " + restriccionTiquetes + "\n";
                  errores[8] = error;
                  bErrorTiquete = true;
                  result = false;
        	}        	
        ...
        if(mostrarValorTotalSol){        	
        	if (mov.getValorTotalMovilidadSolicitado() == null) {
                String error = "El valor total de la movilidad es obligatorio \n";
                errores[8] = error;
                bErrorTiquete = true;
                result = false;
            } else if (mov.getValorTotalMovilidadSolicitado().longValue() > restriccionTiquetes) {
                String error = "El valor total de la movilidad no puede ser superior a " + restriccionTiquetes + "\n";
                errores[8] = error;
                bErrorTiquete = true;
                result = false;
        ...
        if (mostrarSubModalidades) {
        	for(int i = 0; i < listaArchivosObligatorios.size(); i++){
            	...
            	String submodalidadArchivo = "";
            	if(mam.getSubmodalidad() != null){        		
            		submodalidadArchivo = mam.getSubmodalidad();
            	}            	
            	if(submodalidadArchivo.contains(submodalidad) && mam.getEsObligatorio().equals(1L)){
        ...
        for (int j = 0; j < listaArchivosObligatoriosSel.size(); j++) {
        	ArchivoMovilidadDE mva = (ArchivoMovilidadDE) listaArchivosObligatoriosSel.get(j);
    ...
    public void validarFecha(ValueChangeEvent event) {
        ...
        if (diff >= tiempoValidacionMovilidad) {
            mostrarMasDatos();
        } else {
            ocultarMasDatos();
        }
    ...
    public void validarFechaLlegada(ValueChangeEvent event) {
        ...
        if(fechaInicioEvento != null){
        	...
        	mensajeError("Por favor seleccionar la fecha de inicio del evento");
        }
    ...
    public void setListaArchivosObligatoriosSel(List<ArchivoMovilidadDE> listaArchivosObligatoriosSel) {
	...
	/**
	 * @param esEstancia the esEstancia to set
	 */
