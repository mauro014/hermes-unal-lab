...
public class ManejadorCrearEditarMovilidadEventoDoctorado extends ManejadorBase {
	...
	public ManejadorCrearEditarMovilidadEventoDoctorado() {
		ocultarPaneles();
		...
		listaTipoPonencia = servicioGeneral
				.obtenerListaObjetos("TipoPonencia where mostrar = 'S' ");
	...
	public void eliminarPonencia() {
		listaPonencias.remove(tablaPonencias.getRowIndex());
	}

	private void cargarTiposDocumentos() {
		listaArchivosObligatorios = new ArrayList();
		listaArchivosObligatorios = servicioGeneral
				.obtenerListaArchivosMovilidad("B2");
		...
		try {
			...
			if (archivoObligatorio.getContents() != null) {
				...
				listaTipoMovilidad = servicioGeneral
						.obtenerListaObjetos("TipoMovilidad where id ='B2'");
				...
				listaMovilidadArchivo = servicioGeneral
						.obtenerListaArchivosMovilidad("B2", new Long(
								tipoDocumentoSel));
	...
	public void adicionarPonencia() {
		...
		// archivoMovilidad.setNombre(archivoCargar.getFileName().substring(i +
		// 1));
		// archivoMovilidad.setFecha(new Date());
		// archivoMovilidad.setTipoMovilidad(tipoMovilidad);
	...
	public void verArchivo() {
		...
		// listaInformes =
		// servicioGeneral.obtenerListaObjetos("ProyectoInforme  where id ='"+pin.getId()+"'");
	...
	public void buscarPersona() throws SQLException {
		...
		id.setDocumento(persona.getId().getDocumento());
		id.setTipoDocumento(persona.getId().getTipoDocumento());
		...
		/*
		 * sSql = " SELECT COUNT(1) " + " FROM " +
		 * " HER_MOVILIDAD_DOCENTES_EVENTOS " + " WHERE " + " MOV_ID_PER ='" +
		 * documento + "'" + " AND MOV_APROB = 'SI' " +
					...
					if (persona instanceof InvestigadorInterno) {
						...
						if (investigadorInterno.getTipoDedicacion() != null
								&& (investigadorInterno.getTipoDedicacion()
										...
										.equals(Investigador.MEDIOTIEMPO))) {
	...
	public void buscarEstudiante() throws SQLException {
		...
		try {
			List est = servicioGeneral.obtenerListaObjetosWhere(
					"Estudiante e",
					"where e.id.documento='" + documento
			...
			if (nExiste > 0) {
				...
				if (estudiante != null) {
					...
					if (estudiante.getPlan() != null
							&& estudiante.getPlan().getTipo()
									.compareTo(new Long("7")) == 0) {

						programaPosgrado = estudiante.getPlan().getNombre();
						dependencia = estudiante.getDependencia();// servicioPersona
						// .obtenerDependencia(estudiante.getgetId());
						String nombre1, nombre2, apellido1, apellido2;
						...
						noExiste = "El estudiante no pertenece a un programa de Doctorado.";
						mostrarPanelNoExiste();
					}
	...
	public void guardar() {
		...
		/*
		 * movilidad.setIdcreador(Long.valueOf(idcreador));
		 * movilidad.setTipoDocumentoCreador(tipoDocCreador);
		 * movilidad.setIdsubtipo("B1"); Calendar actual =
		 * Calendar.getInstance(); Date date = actual.getTime();
		 * movilidad.setFechasolicitud(date);
		 * //movilidad.setIdpersona(Long.valueOf(documentoDocente));
		 * movilidad.setTipoDocumentoPersona(tipoDocDocente);
		 * //movilidad.setIdproyecto(new Integer(idProyecto)); //
		 * movilidad.setIdplanAccion(new Long(idPlanAccion));
		 * movilidad.setEvento(nombreEvento);
		 * movilidad.setTitulo(tituloPonencia); movilidad.setPais(idPais);
		 * movilidad.setCiudad(ciudadEvento);
		 * movilidad.setCostotiquete(costoTiquetes);
		 * movilidad.setResumen(resumenPonencia);
		 * movilidad.setInscripcionevento(solInscripcion);
		 * movilidad.setTipoPonencia(idTipoPonencia);
		 * movilidad.setOtroTipoPonencia(otroTipoPonencia);
		 * movilidad.setCostoevento(costoInscripcion);
		 * movilidad.setAportefacultad(aporteFacultad);
		 * movilidad.setResolucionviaje(aceptacionPonencia);
		 * movilidad.setFechainicial(fechaInicioEvento);
		 * movilidad.setFechafinal(fechaFinEvento);
		 * movilidad.setAceptacion(aceptacionFacultad);
		 */
		...
		mde.setInscripcionevento(solInscripcion);
		...
		mde.setTesis(nombreTesis);
		mde.setInscripcionevento("SI");

		Persona personaRegistra = new Persona();

		personaRegistra = (Persona) sesion.getAttribute("persona");

		mde.setIdReg(personaRegistra.getId().getDocumento());
		mde.setTipoDocumentoReg(personaRegistra.getId().getTipoDocumento());
		...
		List listaPonencia = new ArrayList();
		listaPonencia = servicioGeneral
				.obtenerListaObjetos("TipoPonencia where id ='5'");
		...
		if (validar(mde)) {
			...
			/*
			 * List listaProyecto = new ArrayList(); listaProyecto =
			 * servicioGeneral .obtenerListaObjetos("Proyecto where id ='" +
			 * idProyecto + "'"); Proyecto proyecto = new Proyecto(); proyecto =
			 * (Proyecto) listaProyecto.get(0);
			 * 
			 * mde.setProyecto(proyecto);
			 */
			...
			/*
			 * if (listaArchivos.size() > 0) { for (int i = 0; i <
			 * listaArchivos.size(); i++) { ArchivoMovilidadDE amovAux =
			 * (ArchivoMovilidadDE) listaArchivos .get(i);
			 * amovAux.setMovilidad(mde);
			...
			editarCorreo(personaActual, mde);
			...
			if (listaParametro == null || listaParametro.size() == 0) {
				...
				String consult = "select i from InvestigadorInterno i, "
						...
						+ " and pr.nombre = 'MF' and i.dependencia2.id = '"
						+ dependencia.getId() + "' "
	...
	public String editarCorreo(Persona personaAux, MovilidadDocentesExterior mov) {
		...
		try {
			...
			List listaTipoMovilidadAux = new ArrayList();
			listaTipoMovilidadAux = servicioGeneral
					.obtenerListaObjetos("TipoMovilidad where id ='B2'");
			TipoMovilidad tipoMovilidad2 = (TipoMovilidad) listaTipoMovilidadAux
					.get(0);

			correo = correo.replaceAll("<<IDMOVILIDAD>>", mov.getId()
					.toString());
			correo = correo.replaceAll("<<TIPO>>", tipoMovilidad2.getNombre());
		...
		try {
			...
			if (archivoVicerectoria != null) {
				...
				try {
					...
					// archivoCuadroResultados.getContents());
	...
	private boolean validar(MovilidadDocentesExterior mov) {
		...
		/*
		 * if (idProyecto.equals("0")) { // if
		 * (mov.getIdplanAccion().longValue() == 0) { String error =
		 * "Se debe seleccionar un proyecto. \n"; bErrorProyecto = true;
		 * errores[0] = error; result = false; // } }
		 */
	...
	public void apruebaEntidad() {
		...
		// listaInformes = new ArrayList();
		// listaInformes =
		// servicioGeneral.obtenerListaObjetos("ProyectoInforme  where id ='"+idInforme+"'");
		// ProyectoInforme pinAux = new ProyectoInforme();
	...
	public void guardarArchivo() {
		...
		try {
			...
			if (archivoCargar.getContents() != null) {
				...
				List listaTipoMovilidad = new ArrayList();
				listaTipoMovilidad = servicioGeneral
						.obtenerListaObjetos("TipoMovilidad where id ='B1'");
				TipoMovilidad tipoMovilidad = (TipoMovilidad) listaTipoMovilidad
						.get(0);
	...
	/**
	 * @param fechaMinimaInicio the fechaMinimaInicio to set
	 */
