...
public class ManejadorEditarFacultadMovilidadVisArt extends ManejadorBase {
	...
	public ManejadorEditarFacultadMovilidadVisArt() {
	...
	public void listarMovilidades() {
		...
		//List listaMovilidades = servicioGeneral
			//	.obtenerObjetos("Select m from MovilidadVisitantesArtes m, InvestigadorInterno i where m.personaInv.id.documento = i.id.documento"
				//			+ " and i.dependencia.sede.id = "+ dependencia.getSede().getId() +" and m.aceptacionFacultad is null ORDER BY m.id");
		
		List listaMovilidades = servicioGeneral
		.obtenerObjetos("Select m from MovilidadVisitantesArtes m where m.aceptacionFacultad is null and m.estado = 'P' ORDER BY m.id");
	...
	public void avalAprobar(){
		...
		if(this.avalDescripcion == null || this.avalDescripcion.length() <= 0){
			...
			.obtenerListaObjetos("MovilidadVisitantesArtes m where m.id = '"+ this.movArt.getId()+ "'");
			if (objeto != null && objeto.size() > 0) {
				...
				try {
					String sql = "update HER_MOVILIDAD_VISITANTES_ART set MOV_AVAL_FACULTAD = 'SI', MOV_DESCRIPCION_FACULTAD = '"+this.avalDescripcion+"'  where MOV_ID = '"+ movilidad.getId()+"'";
				...
				sesion.removeAttribute("ManejadorEditarSedeMovilidadVisArt");
    ...
    public void avalNegar(){
    	...
    	if(this.avalDescripcion == null || this.avalDescripcion.length() <= 0){
			...
			if (objeto != null && objeto.size() > 0) {
				...
				try {
					String sql = "update HER_MOVILIDAD_VISITANTES_ART set MOV_AVAL_FACULTAD = 'NO', MOV_DESCRIPCION_FACULTAD = '"+this.avalDescripcion+"'  where MOV_ID = '"+ movilidad.getId()+"'";
					servicioGeneral.eliminar(sql);
				} catch (SQLException e) {
					System.out.println(e.toString());
				} 
	...
	public void buscarPersona() throws SQLException {
		...
		if (objeto != null && objeto.size() > 0) {
			...
			if(this.movArt.getConvenio()!= null && this.movArt.getConvenio().endsWith("SI")){
				this.banderaConvenio = true;
			}else{
				this.banderaConvenio = false;
			}
	...
	public void guardar() {
		...
		if (validacion() && validarPresupuesto()) {
			...
			this.movArt.setSedeFrontera(this.movArt.getSedeFrontera()
					.toUpperCase());
			...
			if(this.movArt.getConvenio().equals("NO")){
				this.movArt.setTiquetesP(null);
				this.movArt.setTallerP(null);
				this.movArt.setAlojamientoP(null);
				this.movArt.setAlimentacionP(null);
				this.movArt.setMaterialesP(null);
				this.movArt.setTransporteP(null);
				this.movArt.setSocializacionP(null);
				this.movArt.setTotalPropio(null);
			}
	...
	private void cargarValoresIniciales() {
		...
		convenio = new Vector();
		convenio.add(new SelectItem("NO", "NO"));
		convenio.add(new SelectItem("SI", "SI"));
