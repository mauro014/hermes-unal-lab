...
public class ManejadorEditarFacultadMovilidadEstArt extends ManejadorBase {
	...
	public ManejadorEditarFacultadMovilidadEstArt() {
	...
	public void listarMovilidades() {
		...
		List listaMovilidades = servicioGeneral
				.obtenerObjetos("Select m from MovilidadEstudiantesArtes m, InvestigadorInterno i where m.personaInv.id.documento = i.id.documento"
							+ " and i.dependencia.sede.id = "+ dependencia.getSede().getId() +" and m.aceptacionFacultad is null and m.estado = 'P' ORDER BY m.id");
		...
		if (listaMovilidades != null && listaMovilidades.size() > 0) {
			...
			for (int i = 0; i < listaMovilidades.size(); i++) {
				MovilidadEstudiantesArtes movilidad = (MovilidadEstudiantesArtes) listaMovilidades
						.get(i);
				movilidadItem[i] = new SelectItem(movilidad.getId(), "Id="
						+ movilidad.getId() + " - Fecha Registro("
						+ movilidad.getFechasolicitud() + ")");
			}
			codigoMovilidad = ((MovilidadEstudiantesArtes) listaMovilidades.get(0))
					.getId();
		} else {
	...
	private void cargarTiposDocumentos() {
		...
		listaArchivosObligatorios = servicioGeneral
				.obtenerListaArchivosMovilidad("C2");
	...
	public void avalAprobar(){
		...
		if(this.avalDescripcion == null || this.avalDescripcion.length() <= 0){
			...
			.obtenerListaObjetos("MovilidadEstudiantesArtes m where m.id = '"+ this.movArt.getId()+ "'");
			if (objeto != null && objeto.size() > 0) {
				MovilidadEstudiantesArtes movilidad = (MovilidadEstudiantesArtes) objeto.get(0);
				movilidad.setAceptacionFacultad("SI");
				movilidad.setDescripcionFacultad(this.avalDescripcion);
				try {
					String sql = "update HER_MOVILIDAD_ESTUDIANTES_ART set MOV_AVAL_FACULTAD = 'SI', MOV_DESCRIPCION_FACULTAD = '"+this.avalDescripcion+"'  where MOV_ID = '"+ movilidad.getId()+"'";
				...
				String correoDIB = "";
				List listaParametroDIB = this.servicioGeneral.obtenerObjetos("FROM Parametro WHERE id = 69");	
				if (listaParametroDIB != null && listaParametroDIB.size() > 0) {
					Parametro par = (Parametro) listaParametroDIB.get(0);
					correoDIB = par.getValor();
				}
				
				CorreoPlantilla correoActual = new CorreoPlantilla();
				String cuerpoCorreo = "";
				correoActual = cargarPlantilla(81);
				...
				this.limpiar();
				listarMovilidades();
				this.errores[0] = "Aval Local guardado satisfactoriamente.";
				this.panelRenderError[0] = true;
				
				sesion.removeAttribute("ManejadorEditarSedeMovilidadEstArt");
			}
		}
	}
    ...
    public void avalNegar(){
    	...
    	if(this.avalDescripcion == null || this.avalDescripcion.length() <= 0){
			...
			if (objeto != null && objeto.size() > 0) {
				MovilidadEstudiantesArtes movilidad = (MovilidadEstudiantesArtes) objeto.get(0);
				movilidad.setAceptacionFacultad("NO");
				movilidad.setDescripcionFacultad(this.avalDescripcion);
				try {
					String sql = "update HER_MOVILIDAD_ESTUDIANTES_ART set MOV_AVAL_FACULTAD = 'NO', MOV_DESCRIPCION_FACULTAD = '"+this.avalDescripcion+"'  where MOV_ID = '"+ movilidad.getId()+"'";
					servicioGeneral.eliminar(sql);
				} catch (SQLException e) {
					System.out.println(e.toString());
				} 
				...
				CorreoPlantilla correoActual = new CorreoPlantilla();
				String cuerpoCorreo = "";
				correoActual = cargarPlantilla(83);
				cuerpoCorreo = editarCorreo(this.movArt.getPersonaInv(), String.valueOf(this.movArt.getId()), this.movArt.getTipoMovilidad().getNombre(), correoActual.getCuerpo());
				...
				this.limpiar();
				listarMovilidades();
				this.errores[0] = "Aval facultad guardado satisfactoriamente.";
				this.panelRenderError[0] = true;
	...
	public void buscarPersona() throws SQLException {
		...
		List objeto = this.servicioGeneral
		.obtenerListaObjetos("MovilidadEstudiantesArtes m where m.id = '"
				+ codigoMovilidad + "'");
		...
		movArt.setPersonaInv(servicioPersona.obtenerPersonaRoles(personaActual.getId()));
		*/
			personaActual = servicioPersona.obtenerPersonaRoles(this.movArt.getPersonaInv().getId());
			if (this.personaActual != null) {
				...
				if (this.personaActual instanceof Investigador) {
					if (this.personaActual instanceof InvestigadorInterno) {
						this.personaActual = servicioPersona.obtenerInvestigadorInternoCompleto(this.personaActual.getId());
						InvestigadorInterno investigadorInterno = (InvestigadorInterno) this.personaActual;
						Dependencia dependencia,dependenciae;
						
						String nombre1, nombre2, apellido1, apellido2,nombre1e, nombre2e, apellido1e, apellido2e;
						...
						if (investigadorInterno.getTipoDedicacion() != null
							...
							List est = servicioGeneral.obtenerListaObjetosWhere("Estudiante e",
									"where e.id.documento='" + movArt.getEstudianteInv().getId().getDocumento()
											+ "' and e.id.tipoDocumento='"
											+ movArt.getEstudianteInv().getId().getTipoDocumento() + "'");
							if (est != null && est.size() > 0) {
								...
								if (estudianteActual.getNombre1() != null) {
									nombre1e = estudianteActual.getNombre1();
								} else {
									nombre1e = "";
								}
								if (estudianteActual.getNombre2() != null) {
									nombre2e = estudianteActual.getNombre2();
								} else {
									nombre2e = "";
								}
								if (estudianteActual.getApellido1() != null) {
									apellido1e = estudianteActual.getApellido1();
								} else {
									apellido1e = "";
								}
								if (estudianteActual.getApellido2() != null) {
									apellido2e = estudianteActual.getApellido2();
								} else {
									apellido2e = "";
								}
								...
								this.nombreEstudiante = nombre1e + " " + nombre2e + " "
								+ apellido1e + " " + apellido2e;

								this.documentoDocente = investigadorInterno.getId()
										.getDocumento();
								
								this.documentoEstudiante = estudianteActual.getId()
								.getDocumento();
								
								dependenciae = estudianteActual.getDependencia();
								...
								if (dependenciae!=null && dependenciae.getFacultad() != null) {
									this.sedeEstudiante = dependenciae.getSede().getNombre();
									this.facultadEstudiante = dependenciae
											.getFacultad().getNombre();
									this.departamentoEstudiante = dependenciae
											.getNombre();	
									...
									if (estudianteActual.getPlan() != null && ( (estudianteActual.getPlan().getTipo().compareTo(new Long("4"))==0)) || (estudianteActual.getPlan().getTipo().compareTo(new Long("5"))==0) || (estudianteActual.getPlan().getTipo().compareTo(new Long("6"))==0) || (estudianteActual.getPlan().getTipo().compareTo(new Long("7"))==0)){
									...
									estudianteActual = new Estudiante();
									errores[0] = "La dependencia del estudiante no tiene una facultad asociada";
									panelRenderError[0] = true;
									ocultarPaneles(0);
								}
						...
						errores[0] = "El documento del investigador ingresado no corresponde a un investigador";
						panelRenderError[0] = true;
						ocultarPaneles(0);
					}
				} else {				
				...
				errores[0] = "El documento del investigador ingresado no existe";
				panelRenderError[0] = true;
				ocultarPaneles(0);
			}
		}
	...
	public void buscarEstudiante() throws SQLException {
		...
		sSql = " SELECT COUNT(1) " + " FROM "
				...
				+ " MOV_ID_EST ='" + estudiante.getId().getDocumento() + "'" + " AND MOV_APROB = 'SI' "
	...
	public String getDepartamentoEstudiante() {
		return departamentoEstudiante;
	}

	public void setDepartamentoEstudiante(String departamentoEstudiante) {
