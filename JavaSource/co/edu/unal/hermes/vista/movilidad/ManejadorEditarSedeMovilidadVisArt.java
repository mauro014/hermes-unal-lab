...
public class ManejadorEditarSedeMovilidadVisArt extends ManejadorBase {
	...
	public ManejadorEditarSedeMovilidadVisArt() {
	...
	public void listarMovilidades() {
		...
		List listaMovilidades = servicioGeneral
				.obtenerObjetos("Select m from MovilidadVisitantesArtes m where m.dependencia =  " + dependencia.getSede().getId() +" and m.aceptacionFacultad = 'SI' and m.aprobacionSede is null ORDER BY m.id");
	...
	public void avalAprobar(){
		...
		if(this.avalDescripcion == null || this.avalDescripcion.length() <= 0){
			...
			if (objeto != null && objeto.size() > 0) {
				...
				try {
					String sql = "update HER_MOVILIDAD_VISITANTES_ART set MOV_AVAL_SEDE = 'SI', MOV_DESCRIPCION_SEDE = '"+this.avalDescripcion+"'  where MOV_ID = '"+ movilidad.getId()+"'";
    ...
    public void avalNegar(){
    	...
    	if(this.avalDescripcion == null || this.avalDescripcion.length() <= 0){
			...
			if (objeto != null && objeto.size() > 0) {
				...
				movilidad.setDescripcionSede(this.avalDescripcion);
				try {
					String sql = "update HER_MOVILIDAD_VISITANTES_ART set MOV_AVAL_SEDE = 'NO', MOV_DESCRIPCION_SEDE = '"+this.avalDescripcion+"'  where MOV_ID = '"+ movilidad.getId()+"'";
					servicioGeneral.eliminar(sql);
				} catch (SQLException e) {
					System.out.println(e.toString());
				} 
