...
public class ManejadorCrearMovilidadPosgradoInvConvFac extends
ManejadorBase {

    private static final long serialVersionUID = 3523960358928354884L;
	...
	public ManejadorCrearMovilidadPosgradoInvConvFac() {
		ocultarPaneles();
		...
		listaTipoPonencia = servicioGeneral.obtenerListaObjetos("TipoPonencia where id in (2, 12, 13, 3, 11)");
	...
	public void activarOtrotipoPonencia() { // ValueChangeEvent event) {
		...
		if (tipoPonenciaActual.equals("10") || tipoPonenciaActual.equals("11")) {
			esPonencia = false;
			if(tipoPonenciaActual.equals("11")){
				esPasantia = true;
			}else{
				esPasantia = false;
			}
	...
	public void buscarEstudiante() throws SQLException {
		...
		sSql = " SELECT COUNT(1) " + " FROM "
				+ " HER_MOVILIDAD_ESTUDIANTE_POS " + " WHERE "
				+ " MOV_ID_EST ='" + estudiante.getId().getDocumento() + "'"
				+ " AND MOV_APROB = 'SI' "
		...
		if (nExiste < 0) {
			...
			if (estudiante != null) {
				...
				nombreDocente = nombre1 + " " + nombre2 + " " + apellido1 + " "
						+ apellido2;
				...
				if (papaEstudiante < 4.0) {/*
					String error = "El Promedio aritmético del estudiante "
							+ nombreDocente + " es " + papaEstudiante
				...
				if (persona instanceof InvestigadorInterno) {
					...
					if (investigadorInterno.getTipoDedicacion() != null
						...
						if (dependencia.getFacultad() != null) {
							...
							departamentoDocente = dependencia.getNombre();
							/*
							 * Investigador inv; inv =
							 * servicioPersona.obtenerInvestigadorProyectos
							 * (investigadorInterno.getId()); Set proyectos =
							 * inv.getProyectosInvestigador(); Iterator it =
							 * proyectos.iterator(); proyectosInvestigador = new
							 * Vector(); SelectItem stmp = new
							 * SelectItem("0","Seleccione un proyecto");
							 * proyectosInvestigador.add(stmp); while
							 * (it.hasNext()) { InvestigadorProyecto p =
							 * (InvestigadorProyecto) it.next(); if
							 * (p.getProyecto
							 * ().getEstadoProyecto().getId().equals("A")) {
							 * SelectItem s = new SelectItem(String.valueOf(p
							 * .getProyecto().getId
							 * ()),p.getProyecto().getNombre());
							 * proyectosInvestigador.add(s); } else if
							 * (p.getProyecto().getEstadoProyecto()
							 * .getId().equals("AP")) { SelectItem s = new
							 * SelectItem(String
							 * .valueOf(p.getProyecto().getId()),
							 * p.getProyecto().getNombre());
							 * proyectosInvestigador.add(s); } else if
							 * (p.getProyecto().getEstadoProyecto()
							 * .getId().equals("F")) { SelectItem s = new
							 * SelectItem(String
							 * .valueOf(p.getProyecto().getId()),
							 * p.getProyecto().getNombre());
							 * proyectosInvestigador.add(s); } }
							 */

							// if (proyectosInvestigador.size() == 1) {
							// noExiste =
							// "El investigador no tiene proyectos asociados";
							// mostrarPanelNoExiste();
							// } else {
							mostrarDatosPersona();
							// }
	...
	public void guardarConvocatoriaFacultad(){
		...
		mep.setCostoevento(costoInscripcion);
		mep.setUniversidad(universidad);
		mep.setValorViaticos(valorViaticos);
		mep.setValorOtrosAportes(valorOtrosAportes);
		...
		listaTipoMovilidad = servicioGeneral.obtenerListaObjetos("TipoMovilidad where id ='CF_MOV1'");			
		...
		if (validar(mep)) {
			...
			if (listaArchivosObligatoriosSel != null
				...
				for (int i = 0; i < listaArchivosObligatoriosSel.size(); i++) {
					ArchivoMovilidadEP amovAux1 = (ArchivoMovilidadEP) listaArchivosObligatoriosSel
							.get(i);
			...
			personaEnvio = servicioPersona
					.obtenerInvestigadorInternoCompleto(mep.getPersonaInv()
							.getId());
			...
			if (listaCorreoEncargado != null && listaCorreoEncargado.size() > 0) {
				...
				for (int i = 0; i < numCoord; i++) {
					...
					if (bandera) {
						...
						paActual = servicioPersona
								.obtenerInvestigadorInterno(id);
					} else {
						paActual = (InvestigadorInterno) listaCorreoEncargado
								.get(i);
					}
...
//			noExiste1 = "Solicitud de movilidad guardada correctamente, con el número "
//					+ mep.getId()
//					+ ".  Se ha enviado a su correo electrónico un mensaje confirmando el registro.";
	...
	public String editarCorreo(Persona personaAux,
			MovilidadEstudiantesPosgrado mov) {
	...
	private boolean validar(MovilidadEstudiantesPosgrado mov) {
		...
		if ((mov.getTitulo() == null || mov.getTitulo().equals(""))
				&& esPonencia) {
		...
		if ((mov.getResumen() == null || mov.getResumen().equals(""))
				&& esPonencia) {
		...
		if (listaArchivosObligatoriosSel == null
			...
			for (int j = 0; j < listaArchivosObligatoriosSel.size(); j++) {
				...
				ArchivoMovilidadEP mva = (ArchivoMovilidadEP) listaArchivosObligatoriosSel
						.get(j);
	...
	private void cargarTiposDocumento() {
		...
		List listaTipoDocumentoEstudiante = servicioGeneral
				.obtenerListaObjetos("TipoDocumento");

		tipoDocumentoEstudianteItem = new SelectItem[listaTipoDocumentoEstudiante
		                                             .size()];
		for (int i = 0; i < listaTipoDocumentoEstudiante.size(); i++) {
			TipoDocumento td1 = (TipoDocumento) listaTipoDocumentoEstudiante
					.get(i);
			tipoDocumentoEstudianteItem[i] = new SelectItem(td1.getId(),
					td1.getNombre());
		}
		tipoDocumentoEstudiante = (TipoDocumento) listaTipoDocumentoEstudiante
				.get(0);
	...
	private void cargarTiposDocumentos() {
		listaArchivosObligatorios = new ArrayList();
		listaArchivosObligatorios = servicioGeneral.obtenerListaArchivosMovilidad("CF_MOV1");
		...
		try {
			...
			if (archivoObligatorio.getContents() != null) {
				...
				listaTipoMovilidad = servicioGeneral
						.obtenerListaObjetos("TipoMovilidad where id ='CF_MOV1'");
				...
				listaMovilidadArchivo = servicioGeneral
						.obtenerListaArchivosMovilidad("CF_MOV1", new Long(
								tipoDocumentoSel));
	...
	public void eliminarArchivoObligatorio() {
		 ...
		 * listaTipoArchivo = servicioGeneral
		 * .obtenerListaObjetos("TipoArchivoMovilidad where id ='" +
		 * amv.getTipoArchivo().getId() + "'"); TipoArchivoMovilidad tipoArchivo
		 * = (TipoArchivoMovilidad) listaTipoArchivo .get(0);
		 * 
	...
	public void setTipoDocumentoEstudianteItem(
			SelectItem[] tipoDocumentoEstudianteItem) {
	 ...
	 */
	public void setArchivoMovilidadEPSeleccionado(
			ArchivoMovilidadEP archivoMovilidadEPSeleccionado) {
