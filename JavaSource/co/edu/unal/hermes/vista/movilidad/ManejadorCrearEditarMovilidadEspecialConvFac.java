...
public class ManejadorCrearEditarMovilidadEspecialConvFac extends ManejadorBase {
	...
	public ManejadorCrearEditarMovilidadEspecialConvFac() {
		ocultarPaneles();
		...
		if(sesion.getAttribute("idConvocatoriaActual") != null){
			...
			if (listConvocatorias.size() > 0 && listConvocatorias != null) {
				...
				if(conv.getRestriccion().getId() != null && conv.getRestriccion().getId().equals("CF_MOV4")){
					idModMovi = "CF_MOV4";					
				}else{
					idModMovi = "CF_MOV4";	
				}
...
//==========================CAMBIO DE LISTA DE PONENCIAS======================================
	...
	private void cargarTiposDocumentos() {
		listaArchivosObligatorios = new ArrayList();
		listaArchivosObligatorios = servicioGeneral.obtenerListaArchivosMovilidad("CF_MOV4");
		
		//listaArchivosObligatorios = servicioGeneral.obtenerListaArchivosMovilidad("MOV2_IN");
		...
		try {
			...
			if (archivoObligatorio.getContents() != null) {
				...
				listaTipoMovilidad = servicioGeneral
						.obtenerListaObjetos("TipoMovilidad where id ='CF_MOV4'");
				...
				listaMovilidadArchivo = servicioGeneral
						.obtenerListaArchivosMovilidad("CF_MOV4", new Long(
								tipoDocumentoSel));
	...
	public void eliminarArchivoObligatorio() {
		...
		listaMovilidadArchivo = servicioGeneral.obtenerListaArchivosMovilidad(
				"CF_MOV4", amv.getTipoArchivo().getId());
	...
	public void adicionarPonencia() {
		...
		if (validarTituloYEvento(mde)) {
			...
			// archivoMovilidad.setNombre(archivoCargar.getName().substring(i +
			// 1));
			// archivoMovilidad.setFecha(new Date());
			// archivoMovilidad.setTipoMovilidad(tipoMovilidad);
...
//			noExiste1 = "Solicitud de movilidad guardada correctamente. Un correo electrónico confirmando su registro se ha enviado a su cuenta de correo electrónico.";
	...
	private boolean validar(MovilidadDocentesExterior mov) {
		 ...
		 * else{ int suma; suma= 0; for (int i = 0; i < listaPonencias.size();
		 * i++) { MovilidadPonencia mop = new MovilidadPonencia();
		 * mop=(MovilidadPonencia)listaPonencias.get(i); if
		 * (mop.getPonencia().getId().toString().equals("1") ||
		 * mop.getPonencia().getId().toString().equals("4") ||
		 * mop.getPonencia().getId().toString().equals("7")){ suma= suma+1; } }
		 * if(suma == 0){ String error =
		 * "Debe ingresar al menos ponencia oral. \n"; errores[10] = error;
		 * bErrorListaPonencia = true; result = false; }
		 */
		// }
