package co.edu.unal.hermes.vista.editorial.asesor;
...
public class ManejadorAsignacionProyectosEditorial extends ManejadorBase {
	...
	private SelectItem[] coordinadoresItemCon;
	...
	private List<ProyectoCoordinadorEditorial> listaProyectosCoordinadorRev;
	private List<ProyectoCoordinadorEditorial> listaProyectosCoordinadorEv;
	private List<ProyectoCoordinadorEditorial> listaProyectosCoordinadorCon;
	...
	private List listaProyectosCoordinadorAux4;
	...
	private String perIdConcepto;
	private String perNombreSeguim;
	private String perNombreConcepto;
	private Persona personaConcepto;
	...
	public ManejadorAsignacionProyectosEditorial() {
		...
		listaProyectosCoordinadorAux4 = new ArrayList();
	...
	public void consultarConvocatorias() {
		...
		if (nombreConvocatoria != null && nombreConvocatoria.length() > 0) {
			listaConvocatoriasPadre = servicioGeneral.obtenerListaObjetos(
					"ConvocatoriaPadre cp where upper(cp.titulo) like '%" + nombreConvocatoria.toUpperCase() + "%' and cp.tipoConvocatoria = '11' order by cp.id");
	...
	private void cargarCoordinadores() {
		...
		listaCoordinadores = servicioPersona.obtenerListaCoordinadoresEditorial(persona.getId().getDocumento());
		...
		setCoordinadoresItemCon(new SelectItem[listaCoordinadores.size() + 1]);
		...
		coordinadoresItemCon[0] = new SelectItem("0", "Seleccione coordinador");
		...
		for (int i = 0; i < listaCoordinadores.size(); i++) {
			...
			coordinadorNuevoItem[i + 1] = new SelectItem(p.getId().getDocumento(),
					p.getNombre1() + " " + p.getApellido1() + " " + p.getApellido2());
			coordinadoresItemCon[i + 1] = new SelectItem(p.getId().getDocumento(),
					p.getNombre1() + " " + p.getApellido1() + " " + p.getApellido2());
		}
	}
	...
	public void buscarProyectosxModalidad() {
		...
		listaProyectosCoordinadorAux4.clear();
		try {
		...
		if (listaProyectosCoordinador != null) {
			...
			for (int i = 0; i < listaProyectosCoordinador.size(); i++) {
				ProyectoCoordinadorEditorial p = (ProyectoCoordinadorEditorial) listaProyectosCoordinador.get(i);
				p.setCoordinadoresAsesor(coordinadoresActuales);
				String perId = ((ProyectoCoordinadorEditorial) (listaProyectosCoordinador.get(i))).getPerId();
				String perRevision = ((ProyectoCoordinadorEditorial) (listaProyectosCoordinador.get(i))).getPerRevision();
				String perEvaluacion = ((ProyectoCoordinadorEditorial) (listaProyectosCoordinador.get(i))).getPerEvaluacion();
				String perConcepto = ((ProyectoCoordinadorEditorial) (listaProyectosCoordinador.get(i))).getPerConcepto();
				...
				if (perConcepto == null) {
					listaProyectosCoordinadorAux4.add("0");
				} else {
					listaProyectosCoordinadorAux4.add(perConcepto);
				}
	...
	public void buscarProyectoxCodigo() {
		...
		if(codigoProyecto != null && codigoProyecto.trim().length() > 0){
			ProyectoCoordinadorEditorial proyecto = new ProyectoCoordinadorEditorial();
			proyecto = servicioProyecto.obtenerProyectoCoordinadorEditorialCodigo(codigoProyecto);
			listaProyectosCoordinador.clear();
			listaProyectosCoordinadorAux.clear();
			listaProyectosCoordinadorAux2.clear();
			listaProyectosCoordinadorAux3.clear();
			...
			if (proyecto != null) {
				...
				if (proyecto.getPerConcepto() != null) {
					setPerIdConcepto(proyecto.getPerConcepto());
					setTipoDoc4(proyecto.getTdoIdConcepto());
					IdPersona pCon = new IdPersona(perIdConcepto, tipoDoc4);
					personaConcepto = servicioPersona.obtenerPersona(pCon);
					setPerNombreEval(personaConcepto.getNombre1() + " " + personaConcepto.getApellido1());
				}
				...
				if (perNombreConcepto == null) {
					listaProyectosCoordinadorAux4.add("0");
				} else {
					listaProyectosCoordinadorAux4.add(perNombreEval);
				}
	...
	public void buscarProyectosxModalidadxSede() {
		...
		try {
			listaProyectosCoordinador.addAll(servicioProyecto
					.obtenerProyectosEditorialCoordinadorxModalidadxSede(buscarModalidadxId(convocatoria), sede, estado));
		} catch (Exception e) {
	...
	public void buscarProyectoId() {
		...
		for (int j = 0; j < listaProyectoCoordinador.size(); j++) {
			ProyectoCoordinadorEditorial p = (ProyectoCoordinadorEditorial) listaProyectoCoordinador.get(j);
			String perId = ((ProyectoCoordinadorEditorial) (listaProyectoCoordinador.get(j))).getPerId();
			String perRevision = ((ProyectoCoordinadorEditorial) (listaProyectosCoordinador.get(j))).getPerRevision();
			String perEvaluacion = ((ProyectoCoordinadorEditorial) (listaProyectosCoordinador.get(j))).getPerEvaluacion();
			if (perId == null) {
				listaProyectosCoordinadorAux.add("0");
			} else {
				listaProyectosCoordinadorAux.add(perId);
	...
	public void guardarAsociacionProyectosCoordinador() {
		...
		try {
			for (int i = 0; i < listaProyectosCoordinador.size(); i++) {
				...
				String pco4 = (String) listaProyectosCoordinadorAux4.get(i);
				ProyectoCoordinadorEditorial pc = (ProyectoCoordinadorEditorial) listaProyectosCoordinador.get(i);

				@SuppressWarnings("deprecation")
				List dato = servicioGeneral.obtenerObjetos(
						"select pc from ProyectoCoordinadorEditorial pc where pc.idProyecto = '" + pc.idProyecto + "'");
				if (!dato.isEmpty()) {
					ProyectoCoordinadorEditorial pcOriginal = (ProyectoCoordinadorEditorial) dato.get(0);
				...
				if (pc.getPerId() != null) {
					if (pco.equals("0") && !pc.getPerId().equals("0")) {
						...
						if (pb != null) {
							String consulta = " from ProyectoCoordinadorEditorial pc where pc.idProyecto = '"
									+ pc.getIdProyecto().toString() + "'";
							List<ProyectoCoordinadorEditorial> lista = servicioGeneral.obtenerObjetos(ProyectoCoordinadorEditorial.class,
									consulta);
							if (lista.size() > 0) {
								servicioProyecto.modificarCoordinadorEnProyectoEditorial(ProyectoDAOHibernate.UPDATE, "S",
										pc.getIdProyecto().toString(), pb.getId(), asesor, "S", false);

							} else {
								servicioProyecto.modificarCoordinadorEnProyectoEditorial(ProyectoDAOHibernate.INSERT, "S",
										pc.getIdProyecto().toString(), pb.getId(), asesor, "S", false);
							}
						}
					}
					if (!pc.getPerId().equals(pco)) {
						if (pc.getPerId().equals("0")) {
							...
							if (lista != null && lista.size() > 0) {
								...
								IdPersona idPersona = new IdPersona(ProyectoCoordinadorEditorial.COORDINADOR_PRUEBAS, "C");
				...
				if (pc.getPerRevision() != null) {
					if (pco2.equals("0") && !pc.getPerRevision().equals("0")) {
						...
						if (pb != null) {
							if (pc.getPerId() != null && !pc.getPerId().equals("0")) {
								servicioProyecto.modificarCoordinadorEnProyectoEditorial(ProyectoDAOHibernate.UPDATE, "R",
										pc.getIdProyecto().toString(), pb.getId(), asesor, "R", false);
							} else {

								String consulta = "select pc from ProyectoCoordinadorEditorial pc where pc.idProyecto = '"
										+ pc.getIdProyecto().toString() + "'";
								...
								if (lista.size() > 0) {
									...
									servicioProyecto.modificarCoordinadorEnProyectoEditorial(ProyectoDAOHibernate.INSERT, "R",
											pc.getIdProyecto().toString(), pb.getId(), asesor, "R", false);
								}
							}
						}
					...
					if (!pc.getPerRevision().equals(pco2)) {
						if (pc.getPerRevision().equals("0")) {
							...
							mensajeAsignacion += servicioProyecto.modificarCoordinadorEnProyectoEditorial(
				...
				if (pc.getPerEvaluacion() != null) {
					...
					if (pco3.equals("0") && !pc.getPerEvaluacion().equals("0")) {
						...
						if (pb != null) {
							if (pc.getPerId() != null && !pc.getPerId().equals("0")) {
								servicioProyecto.modificarCoordinadorEnProyectoEditorial(ProyectoDAOHibernate.UPDATE, "E",
								...
								if (lista.size() > 0) {
									...
									servicioProyecto.modificarCoordinadorEnProyectoEditorial(ProyectoDAOHibernate.INSERT, "E",
											pc.getIdProyecto().toString(), pb.getId(), asesor, "E", false);
								}
							}
						}
				...
				if (pc.getPerConcepto() != null) {

					if (pco4.equals("0") && !pc.getPerConcepto().equals("0")) {
						// EN EL CASO QUE EL COORDINADOR NO ESTE ASIGNADO SE
						// INSERTA
						j++;
						listaProyectosCoordinadorAux4.set(i, pc.getPerConcepto());
						Persona pb = buscarAsesorxDocumento(pc.getPerConcepto());
						if (pb != null) {
							if (pc.getPerId() != null && !pc.getPerId().equals("0")) {
								servicioProyecto.modificarCoordinadorEnProyectoEditorial(ProyectoDAOHibernate.UPDATE, "C",
										pc.getIdProyecto().toString(), pb.getId(), asesor, "C", false);
							} else {
								...
								if (lista.size() > 0) {
									...
									servicioProyecto.modificarCoordinadorEnProyectoEditorial(ProyectoDAOHibernate.INSERT, "C",
											pc.getIdProyecto().toString(), pb.getId(), asesor, "C", false);
								}
							}
						}
					}
					if (!pc.getPerConcepto().equals(pco4)) {
						if (pc.getPerConcepto().equals("0")) {
							// EN EL CASO QUE LA NUEVA ASIGNACION SEA VACIA SE
							// BORRA EL COORDINADOR DEL PROYECTO
							l++;
							listaProyectosCoordinadorAux4.set(i, "0");
							IdPersona idPersona = new IdPersona("", "");
							mensajeAsignacion += servicioProyecto.modificarCoordinadorEnProyectoEditorial(
									ProyectoDAOHibernate.DELETE, "C", pc.getIdProyecto().toString(), idPersona, asesor,
									"E", true);
						} else {
							// EN EL CASO QUE LA NUEVA ASIGNACION SEA DIFERENTE
							// SE ACTUALIZA EL COORDINADOR DEL PROYECTO
	...
	public void asignarCoordinadorConceptoProyectoTodos() {
		for (int i = 0; i < listaProyectosCoordinador.size(); i++) {
			ProyectoCoordinadorEditorial pc = (ProyectoCoordinadorEditorial) listaProyectosCoordinador.get(i);
			if (pc.isCoordinadorConceptoAsociadoAsesor()) {
				pc.setPerConcepto(coordinadorNuevo);
			}
			listaProyectosCoordinador.set(i, pc);
		}
	}
	...
	public void setListaProyectosCoordinadorRev(List<ProyectoCoordinadorEditorial> listaProyectosCoordinadorRev) {
		this.listaProyectosCoordinadorRev = listaProyectosCoordinadorRev;
	}

	public List<ProyectoCoordinadorEditorial> getListaProyectosCoordinadorRev() {
		return listaProyectosCoordinadorRev;
	}

	public void setListaProyectosCoordinadorEv(List<ProyectoCoordinadorEditorial> listaProyectosCoordinadorEv) {
		this.listaProyectosCoordinadorEv = listaProyectosCoordinadorEv;
	}

	public List<ProyectoCoordinadorEditorial> getListaProyectosCoordinadorEv() {
	...
	public SelectItem[] getCoordinadoresItemCon() {
		return coordinadoresItemCon;
	}

	public void setCoordinadoresItemCon(SelectItem[] coordinadoresItemCon) {
		this.coordinadoresItemCon = coordinadoresItemCon;
	}

	public List<ProyectoCoordinadorEditorial> getListaProyectosCoordinadorCon() {
		return listaProyectosCoordinadorCon;
	}

	public void setListaProyectosCoordinadorCon(List<ProyectoCoordinadorEditorial> listaProyectosCoordinadorCon) {
		this.listaProyectosCoordinadorCon = listaProyectosCoordinadorCon;
	}

	public List getListaProyectosCoordinadorAux4() {
		return listaProyectosCoordinadorAux4;
	}

	public void setListaProyectosCoordinadorAux4(List listaProyectosCoordinadorAux4) {
		this.listaProyectosCoordinadorAux4 = listaProyectosCoordinadorAux4;
	}

	public String getPerIdConcepto() {
		return perIdConcepto;
	}

	public void setPerIdConcepto(String perIdConcepto) {
		this.perIdConcepto = perIdConcepto;
	}

	public String getPerNombreConcepto() {
		return perNombreConcepto;
	}

	public void setPerNombreConcepto(String perNombreConcepto) {
		this.perNombreConcepto = perNombreConcepto;
	}

	public Persona getPersonaConcepto() {
		return personaConcepto;
	}

	public void setPersonaConcepto(Persona personaConcepto) {
		this.personaConcepto = personaConcepto;
	}
	...
	public void setTipoDoc4(String tipoDoc4) {
