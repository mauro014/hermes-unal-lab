...
public class ManejadorAsesorEditorial extends ManejadorBase 
{
	 ...
	 */
	public ManejadorAsesorEditorial()
	{
		reiniciarVariables();
		asesor = (Persona)sesion.getAttribute("persona");
		coordinador = new Persona();
		nivelNacional = false;
		cargarCoordinadoresAsesorEditorial();
		cargarDependencias();
	...
	private void cargarCoordinadoresAsesorEditorial()
	{
    	listaCoordinadoresAsesor = servicioPersona.obtenerListaCoordinadoresAsesorEditorial(asesor.getId());   	
    }
	...
	public void guardar()
	{
		try
		{
			...
			servicioPersona.insertarCoordinadorAsesorEditorial(asesor.getId(), idCoordinador, dependencia );
			...
			this.cargarCoordinadoresAsesorEditorial();
	...
	public void buscarCoordinador(){
		mensajeTransaccion = "";
		coordinador = servicioPersona.obtenerCoordinadorNoAsignadoAsesorEditorial(asesor.getId(), docCoordinador);
		if(coordinador.getId().getDocumento() != null){
			...
			if(docCoordinador.equals("")){
				...
				this.errores[1] = "";
				this.panelRenderError[1] = false;
				mensajeTransaccion = "No se ha encontrado ningun usuario con este documento que posea el rol de Coordinador Editorial. Verifique el documento o solicite la asignación del rol.";
			}
		}
	}
