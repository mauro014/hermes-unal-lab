...
public class ManejadorListaPreinscritosECP extends ManejadorBase {
	...
	public ManejadorListaPreinscritosECP() {
		// idCurso = "17559";
		...
		if (cursoActual != null) {
			...
			if (listaGrupos != null && listaGrupos.size() > 0) {
				...
				for (int j = 0; j < listaGrupos.size(); j++) {
					...
					String consulta = "select pp from Preinscripcion_ECP pp where pp.curso.id = "
							+ cursoActual.getId()
							+ " and pp.sedeFecha like '%"
							+ grCur.getDescripcionGrupo() + "%'";
					List lista = servicioGeneral.obtenerObjetos(consulta);
	...
	public void cargarEstadosPreinscripcion() {
		String consulta = "select dd from Dominio d, DominioDetalle dd where d.id = dd.identificador.id and d.tipo like 'ESTADOS_PREINSCRIPCION_ECP' order by dd.descripcion";
		List lista = servicioGeneral.obtenerObjetos(consulta);
