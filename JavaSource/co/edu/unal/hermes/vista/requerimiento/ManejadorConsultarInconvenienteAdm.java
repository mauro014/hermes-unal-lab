...
public class ManejadorConsultarInconvenienteAdm extends ManejadorBase implements Serializable {
	...
	private List listaTodosAsignadosOSolucionados;
	...
	public SelectItem[] prioridadItem = {new SelectItem("Urgente", "Urgente"),
	...
	public SelectItem[] tipoReporteItem = { new SelectItem("0", "Mejora"), new SelectItem("1", "Incnveniente") };
	...
	public ManejadorConsultarInconvenienteAdm() {
		...
		inconvenienteListaCompleta = new ArrayList();
		inconvenienteListaCompleta = servicioGeneral.obtenerObjetos(Requerimiento.class,
				"from Requerimiento r where r.tipo='Inconveniente' and r.estSelRequerimiento = 'Registrado' and r.dependenciaAsociada != 'VRIE' order by r.id");
		if (inconvenienteListaCompleta != null && inconvenienteListaCompleta.size() > 0) {
			numeroInconvenientes = inconvenienteListaCompleta.size();
			inconvenienteListaCompleta = configurarVista(inconvenienteListaCompleta);
		}

		//
		listaTodosAsignadosOSolucionados = new ArrayList();
		listaTodosAsignadosOSolucionados = servicioGeneral.obtenerObjetos(Requerimiento.class,
				"from Requerimiento r where r.tipo='Inconveniente' and r.estSelRequerimiento != 'Registrado' and r.dependenciaAsociada != 'VRIE' order by r.id");
		if (listaTodosAsignadosOSolucionados != null && listaTodosAsignadosOSolucionados.size() > 0) {
			numeroSolucionados = listaTodosAsignadosOSolucionados.size();
			listaTodosAsignadosOSolucionados = configurarVista(listaTodosAsignadosOSolucionados);
		}

		//
		listaReqAsignados = new ArrayList();
		listaReqAsignados = servicioGeneral.obtenerObjetos(Requerimiento.class,
				"from Requerimiento r where r.tipo='Inconveniente' and r.estSelRequerimiento = 'En Desarrollo' and r.dependenciaAsociada != 'VRIE' order by r.id");
		if (listaReqAsignados != null && listaReqAsignados.size() > 0) {
			setNumeroAtendidos(listaReqAsignados.size());
	...
	public void buscarIngeniero() {
		...
		if (this.requerimiento.getIngenieroAsignado() != null) {
			...
			try {
				...
				String hql = "select r from Requerimiento r where r.ingenieroAsignado = '"
						+ requerimiento.getIngenieroAsignado()
						+ "' and r.dependenciaAsociada != 'VRIE' and r.estSelRequerimiento != 'Solucionado'"
						+ "		and r.estSelRequerimiento != 'No aplica'  and r.tipo='Mejora' order by r.id";
				// System.out.println(hql);
				requerimientosAsignadosListaCompleta = servicioGeneral.obtenerObjetos(Requerimiento.class, hql);

				if (requerimientosAsignadosListaCompleta != null && requerimientosAsignadosListaCompleta.size() > 0) {
					mostrarForm = true;
	...
	public String verTodosSolucionados() {
		sesion.removeAttribute("ManejadorConsultarRequerimientoAdm");
	...
	public List getListaTodosAsignadosOSolucionados() {
		return listaTodosAsignadosOSolucionados;
	}

	public void setListaTodosSolucionados(List listaTodosAsignadosOSolucionados) {
		this.listaTodosAsignadosOSolucionados = listaTodosAsignadosOSolucionados;
	}
