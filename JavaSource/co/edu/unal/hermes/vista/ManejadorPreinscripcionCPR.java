...
public class ManejadorPreinscripcionCPR extends ManejadorBase {
	...
	// para los cursos que deban adjuntar archivos
	private SelectItem[] listaArchivosObligatorios;
	private String tipoArchivoAdjuntar;
	private String tipoArchivoAdjuntarDos;
	...
	private static Logger logger = Logger
			.getLogger(ManejadorPreinscripcionCPR.class.getName());
	int numeroTab;

	boolean aceptaTerminos;

	public ManejadorPreinscripcionCPR() {

		mostrarBotonGuardar = false;

		investigador = new Investigador();
		sesion.removeAttribute("ManejadorPreinscripcionCPR");
	...
	public void cargarSedeFechaOpciones()// SEDE_FECHA
	{
		...
		sedeFechaItemList.add(new SelectItem("Bogotá | viernes, 17 de enero de 2014"));
		sedeFechaItemList.add(new SelectItem("Tunja | viernes, 29 de noviembre de 2013"));
		sedeFechaItemList.add(new SelectItem("Neiva | viernes, 17 de enero de 2014"));
		sedeFechaItemList.add(new SelectItem("Florencia | viernes, 29 de noviembre de 2013"));
		sedeFechaItemList.add(new SelectItem("Quibdó | viernes, 29 de noviembre de 2013"));
		sedeFechaItemList.add(new SelectItem("Montería | viernes, 29 de noviembre de 2013"));
		sedeFechaItemList.add(new SelectItem("Sincelejo | viernes, 29 de noviembre de 2013"));

	}

	public void cargarValoresIniciales() {

		cargarListaArchivosObligatorios();
		...
		numDocRepLegal = "";
		tipoDocRepLegal = "C";
		tipoArchivoAdjuntarDos = "Hoja de vida actualizada";
															...
															// curso...
		{
			...
			if (!documento.equals("")) {
												...
												// [ACTUALIZAR]
				{
					...
					sedeFechaSeleccionada = preins_ecp_objeto.getSedeFecha();
		...
		try {
			...
			if (persona != null) {
				...
				List lista = servicioGeneral
						.obtenerListaObjetos("CorreoPlantilla co where co.id = 162");
				correoActual = (CorreoPlantilla) lista.get(0);

				correoActual.setAsunto(correoActual.getAsunto().replaceAll(
						"<<NOMBRE_CURSO>>", curso.getNombre()));

				correoActual.setCuerpo(correoActual.getCuerpo().replaceAll(
						"<<NOMBRE_CURSO>>", curso.getNombre()));
				correoActual.setCuerpo(correoActual.getCuerpo().replaceAll(
						"<<NOMBRE_ASISTENTE>>",
						preins_ecp_objeto.getInvestigador().getNombre1()
								+ " "
								+ preins_ecp_objeto.getInvestigador()
										.getApellido1()));
				...
				correoActual.setCuerpo(correoActual.getCuerpo().replaceAll(
						...
						preins_ecp_objeto.getInvestigador().getTelefono()));
				correoActual
						.setCuerpo(correoActual.getCuerpo().replaceAll(
								"<<TELEFONO_MOVIL>>",
								preins_ecp_objeto.getTel_movil()));
				correoActual.setCuerpo(correoActual.getCuerpo().replaceAll(
						"<<DIRECCION>>",
						preins_ecp_objeto.getInvestigador().getDireccion()));
				correoActual.setCuerpo(correoActual.getCuerpo()
						.replaceAll("<<CIUDAD>>",
								preins_ecp_objeto.getCiudad().getNombre()));
	...
	public void guardarPreinscripcion() throws IOException {
		...
		if (guardarPersonaInvestigadorBasico()) {
			...
			if (documento != "" && primer_nombre != "" && primer_apellido != ""
				...
				boolean CPR_ACGR = false;
				boolean CPR_AVAL = false;
				boolean CPR_CC = false;
				boolean CPR_CCOM = false;
				boolean CPR_CMOT = false;
				boolean CPR_HV = false;
				String mensajeArchivos = "";

				if (id_oferta != -1 && curso != null) {

					if (listaArchivosObligatorios.length > 0) {
						
						for (int i = 0; i < listaArchivos.size(); i++) {
							ArchivosPreinscripcionECP ap = new ArchivosPreinscripcionECP();
							ap = (ArchivosPreinscripcionECP) listaArchivos
									.get(i);
							if (ap.getTipoArchivo().equals("Acta de grado o diploma del título profesional")) {
								CPR_ACGR = true;
							}

							if (ap.getTipoArchivo().equals("Carta de aval de la empresa")) {
								CPR_AVAL = true;
							}

							if (ap.getTipoArchivo().equals("Cédula de ciudadanía")) {
								CPR_CC = true;
							}

							if (ap.getTipoArchivo().equals("Carta de compromiso")) {
								CPR_CCOM = true;
							}

							if (ap.getTipoArchivo().equals("Carta de motivación")) {
								CPR_CMOT = true;
							}

							if (ap.getTipoArchivo().equals("Hoja de vida actualizada")) {
								CPR_HV = true;
							}
						}
						
						if(CPR_ACGR && CPR_AVAL && CPR_CC && CPR_CCOM && CPR_CMOT && CPR_HV){
							
						}else{
							mensajeArchivos = "Aún no ha adjuntando todos los archivos necesarios para la inscripción.";
						}
					...
					if (ins_act == "I") {
						FacesContext
								...
								.addMessage(
										...
										new FacesMessage(
												FacesMessage.SEVERITY_INFO,
												"La informacion ha sido registrada con éxito. " + mensajeArchivos,
												""));
						habilita_guardar = true;
					} else
						FacesContext
								...
								.addMessage(
										...
										new FacesMessage(
												FacesMessage.SEVERITY_INFO,
												"La informacion ha sido actualizada con éxito. " + mensajeArchivos,
												""));
	...
	public void cambiarTipoArchivo() {
		tipoArchivoAdjuntarDos = tipoArchivoAdjuntar;
	}
	...
	public void subirArchivosTipo(FileUploadEvent event) throws IOException {
		if (event.getFile() != null
				&& (tipoArchivoAdjuntarDos != null && !tipoArchivoAdjuntarDos
						.equals(""))) {
			System.out.println(event.getFile().getFileName());
			ArchivosPreinscripcionECP archivo = new ArchivosPreinscripcionECP();
			archivo.setPreinscripcionECP(preins_ecp_objeto);
			...
			archivo.setEsNuevo(true);
			System.out
					.println("archivo a  adjuntar : ******************************"
							+ tipoArchivoAdjuntarDos);
			archivo.setTipoArchivo(tipoArchivoAdjuntarDos);
			...
			FacesContext
					...
					.addMessage(
							...
							new FacesMessage(
									FacesMessage.SEVERITY_ERROR,
									"Por favor seleccione el tipo de archivo y el archivo",
									""));
		}
	...
	private void cargarListaArchivosObligatorios() {

		String consulta = "select ff from DominioDetalle ff, Dominio d where ff.identificador.id = d.id and d.tipo like 'TIPO_ARCHIVO_DIPLOMADO_CPR' ";
		List lista = servicioGeneral.obtenerObjetos(consulta);
		listaArchivosObligatorios = new SelectItem[lista.size()];
		for (int i = 0; i < lista.size(); i++) {
			DominioDetalle paisObjeto = (DominioDetalle) lista.get(i);
			listaArchivosObligatorios[i] = new SelectItem(
					paisObjeto.getDescripcion(), paisObjeto.getDescripcion());
		}
	...
	public void cargarCiudadOriginal() {
	...
	public static void setLogger(Logger logger) {
		ManejadorPreinscripcionCPR.logger = logger;
	}
	...
	public SelectItem[] getListaArchivosObligatorios() {
		return listaArchivosObligatorios;
	}

	public void setListaArchivosObligatorios(
			SelectItem[] listaArchivosObligatorios) {
