 ...
 */
public class TimerTaskSoporteAvales extends GeneralDAOHibernate {
	private static final Long CORREO_PLANTILLA_ALERTA_AVALES = 316L;
...
//	private static final String PUERTO = "25";
	...
	public void execute() {
		enviarAlertasSoporteParticipacionConvocatoriaExternaAval();
	}
	...
	public void enviarAlertasSoporteParticipacionConvocatoriaExternaAval() {

		List<AlertasAutomaticasAval> actividadesAlertas = obtenerAvalesAlertas(2);
		CorreoPlantilla correoPlantilla = obtenerPlantillaCorreoCompleta(CORREO_PLANTILLA_ALERTA_AVALES);

		if (!esListaVacia(actividadesAlertas)) {
			Iterator<AlertasAutomaticasAval> i = actividadesAlertas.iterator();
			while (i.hasNext()) {
				AlertasAutomaticasAval actividad = i.next();

				Correo correo = new Correo();
				String cuerpo = correoPlantilla.getCuerpo();
				String asunto = correoPlantilla.getAsunto();

				cuerpo = cuerpo.replaceAll("<<INVESTIGADOR>>", actividad.getInvestigador());
				cuerpo = cuerpo.replaceAll("<<ID_AVAL>>", actividad.getAviId().toString());
				cuerpo = cuerpo.replaceAll("<<NOMBRE_PROYECTO>>", actividad.getTituloProyecto());

				correo.setOrigen(Correo.CORREO_HERMES);
				correo.adicionarDireccion(actividad.getEmailInvestigador());
				...
				actualizarInfoAlertaAval(actividad.getAviId());
			}
		}
	...
	public static void setTimer(ScheduledTimerTask timer) {
		TimerTaskSoporteAvales.timer = timer;
	}
}